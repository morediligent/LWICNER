...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        gaz_list, batch_word, batch_biword, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, lexicon_label, mask = batchify_with_label(
            instance, data.HP_gpu, True)
        if data.label_type == "lexicon":
            real_label = lexicon_label
        else:
        ...
        # print "tag:",tag_seq
        if data.label_type =="lexicon":
            real_label_alphabet = data.lexicon_label_alphabet
        else:
            real_label_alphabet = data.label_alphabet
        pred_label, gold_label = recover_label(tag_seq, real_label, mask, real_label_alphabet, batch_wordrecover)
...
def batchify_with_label(input_batch_list, gpu, volatile_flag=False):
    ...
    lexicon_labels = [sent[5] for sent in input_batch_list]
    ...
    with torch.no_grad():
        ...
        lexicon_labels_tensor = autograd.Variable(torch.zeros((batch_size, max_seq_len))).long()
        mask = autograd.Variable(torch.zeros((batch_size, max_seq_len))).bool()
    for idx, (seq, biseq, label, lexicon_label, seqlen) in enumerate(zip(words, biwords, labels, lexicon_labels, word_seq_lengths)):
        word_seq_tensor[idx, :seqlen] = torch.LongTensor(seq)
        biword_seq_tensor[idx, :seqlen] = torch.LongTensor(biseq)
        label_seq_tensor[idx, :seqlen] = torch.LongTensor(label)
        lexicon_labels_tensor[idx, :seqlen] = torch.LongTensor(lexicon_label)
    ...
    lexicon_labels_tensor = lexicon_labels_tensor[word_perm_idx]
    ...
    if gpu:
        ...
        lexicon_labels_tensor = lexicon_labels_tensor.cuda()
        char_seq_tensor = char_seq_tensor.cuda()
        char_seq_recover = char_seq_recover.cuda()
        mask = mask.cuda()
    return gaz_list, word_seq_tensor, biword_seq_tensor, word_seq_lengths, word_seq_recover, char_seq_tensor, char_seq_lengths, char_seq_recover, label_seq_tensor, lexicon_labels_tensor, mask


def train(model, data, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        right_token = 0
        whole_token = 0
        # random.shuffle(data.train_Ids) #wwj
        # set model in train model
        model.train()
        model.zero_grad()
        # current only support batch size = 1 to compulate and accumulate to data.HP_batch_size update weights
        batch_size = 1
        batch_id = 0
        ...
        for batch_id in range(total_batch):
            ...
            loss, tag_seq = model.neg_log_likelihood_loss(gaz_list, batch_word, batch_biword, batch_wordlen, batch_char,
                                                          batch_charlen, batch_charrecover, real_label, mask)
            right, whole = predict_check(tag_seq, real_label, mask)
            right_token += right
            whole_token += whole
        ...
        if current_score > best_dev:
            ...
            del dict['lstm.hidden2tag.weight']
            del dict['lstm.hidden2tag.bias']
            del dict['crf.transitions']
            torch.save(dict, model_name)
...
def load_model_decode(model, data, name, gpu, seg=True):
    data.HP_gpu = gpu
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savemodel', default="../model/saved_model.lstmcrf")
    parser.add_argument('--savedset', help='Dir of saved data setting', default="data/save.dset")
    parser.add_argument('--train', default="../WeiboNER/train.all.bmes")
    parser.add_argument('--dev', default="../WeiboNER/dev.all.bmes")
    parser.add_argument('--test', default="../WeiboNER/test.all.bmes")
    ...
    data.build_lexicon_alphabet()
    data.label_type = "lexicon"
    model = SeqModel(data)
    train(model, data, seg)

    data.label_type = "entity"
    model = SeqModel(data)
    ...
    train(model, data, seg)
    data.generate_instance_with_gaz(dev_file, 'dev')
    load_model_decode(model, data, 'dev', gpu, seg)
    data.generate_instance_with_gaz(test_file, 'test')
    load_model_decode(model, data, 'test', gpu, seg)
